...
# 获取 VPS 的外部 IP 地址
VPS_IP=$(curl -s https://api.ipify.org || curl -s http://icanhazip.com)

# 检查是否能够获取到IP
if [ -z "$VPS_IP" ]; then
    echo "无法获取到公网IP地址，请检查网络连接，程序退出。"
    exit 1
fi
...
echo "更新系统并安装基础工具..."
yum update -y
yum install -y epel-release wget net-tools httpd-tools firewalld squid curl
...
configure_firewall() {
    ...
    firewall-cmd --permanent --zone=public --add-port=$SOCKS_PORT/tcp
    ...
    echo "防火墙已配置，只开放端口：22（SSH，可选）、$HTTP_PORT（HTTP）、$SOCKS_PORT（SOCKS5）"
}

# 安装并配置Squid（HTTP和SOCKS5代理，带认证）
install_proxy() {
    echo "安装Squid HTTP和SOCKS5代理..."
    yum install -y squid
    if [ $? -ne 0 ]; then
...
EOF
    ...
    if systemctl is-active squid >/dev/null 2>&1; then
        ...
        echo "SOCKS5代理：$VPS_IP:$SOCKS_PORT (用户：$USERNAME 密码：$PASSWORD)"
    else
...
check_ports() {
    ...
    if netstat -tuln | grep -q ":$SOCKS_PORT "; then
        echo "端口 $SOCKS_PORT 已被占用，请更换端口或释放该端口"
        exit 1
    fi
}
...
main() {
    ...
    echo "HTTP代理：$VPS_IP:$HTTP_PORT (用户：$USERNAME 密码：$PASSWORD)"
    echo "SOCKS5代理：$VPS_IP:$SOCKS_PORT (用户：$USERNAME 密码：$PASSWORD)"
    echo "防火墙已启用，只开放端口：22（SSH，可选）、$HTTP_PORT（HTTP）、$SOCKS_PORT（SOCKS5）"
